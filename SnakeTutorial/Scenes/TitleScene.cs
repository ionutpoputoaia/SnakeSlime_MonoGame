...
public class TitleScene : Scene
{
    ...
    {
        // The SpriteFont used to draw the "Press Enter To Start" text is loaded 
        // using the global content manager.
        // The SpriteFont used to draw the "Dungeon" and "Slime" text is loaded 
        // using the scene's content manager.
    ...
    {
        // If the user presses the exit key, exit the game.
        if (Core.Input.Keyboard.WasKeyJustPressed(Keys.Escape))
        {
            Core.ExitOnEscape = true;
        }

        // Update the offsets for the background pattern wrapping so that
        // it scrolls down and to the right
        float offset = _scrollSpeed * (float)gameTime.ElapsedGameTime.TotalSeconds;
        _backgroundOffset.X -= offset;
        _backgroundOffset.Y -= offset;

        // Ensure that the offsets do not go beyond the texture bounds
